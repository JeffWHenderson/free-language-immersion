...
const StoryList = () => {
    ...
    const regularFiltered = filtered.filter((s) => s.deckId !== CROSS_SECTION_DECK);
    const crossFiltered = filtered.filter((s) => s.deckId === CROSS_SECTION_DECK);

    const uniqueDecks = Array.from(new Set(stories.map((s) => s.deckId)))
        .sort((a, b) => a === CROSS_SECTION_DECK ? 1 : b === CROSS_SECTION_DECK ? -1 : 0)
        .map((id) => ({ id, name: stories.find((s) => s.deckId === id)!.deckName }));
        ...
        <div className="srs-container">
            ...
            <div className="srs-home-header">
                <div className="srs-header-row">
                    <h2>Stories</h2>
                    <InfoTip>
                        <p>Short passages written around each deck's vocabulary.</p>
                        <p>Filter by <strong>Topic</strong> to focus on a specific deck, or by <strong>Difficulty</strong> for easier or harder reading.</p>
                        <p>Reading reinforces words you've seen as flashcards and helps you see them in natural context.</p>
                    </InfoTip>
                </div>
            </div>
            ...
            {crossFiltered.length > 0 && (
                <>
                    <div className="srs-cross-section-header">
                        <h3>Cross-Section Lessons</h3>
                        <p>Longer stories drawing vocabulary from all topics</p>
                    </div>
                    <div className="srs-story-grid">
                        {crossFiltered.map((story) => (
                            <button
                                key={`${story.deckId}-${story.id}`}
                                className="srs-story-card"
                                onClick={() => navigate(`/${language}/${story.deckId}/story/${story.id}`)}
